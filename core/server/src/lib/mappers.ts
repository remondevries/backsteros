...
export function toDocument(row: DbDocument): Document {
  return {
    ...
    publishStatus: (row.publishStatus as Document["publishStatus"]) ?? "concept",
    publishSlug: row.publishSlug ?? null,
    seoTitle: row.seoTitle ?? null,
    seoDescription: row.seoDescription ?? null,
    audience: (row.audience as Document["audience"]) ?? "group",
    contactIds: row.contactIds ?? null,
    placementFolderId: row.placementFolderId ?? null,
    coverStorageKey: row.coverStorageKey ?? null,
    coverContentType: row.coverContentType ?? null,
    createdAt: row.createdAt.toISOString(),
    updatedAt: row.updatedAt.toISOString(),
    deletedAt: toIso(row.deletedAt),
  };
