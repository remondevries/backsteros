/**
 * Geometry + model for task status icons — kept in sync with
 * `backsteros-packages/ui/src/task-status-icon-model.ts`.
 */
...
export function computeTaskStatusIconModel(
  status: string | null | undefined,
): TaskStatusIconModel {
  const normalized = migrateLegacyTaskStatus(status);
  const color = TASK_STATUS_COLORS[normalized];

  if (normalized === "triage") return { kind: "triage", color };
  if (normalized === "backlog") return { kind: "backlog", color };
  if (normalized === "completed") return { kind: "completed", color };
  if (normalized === "duplicated") return { kind: "duplicated", color };

  return {
    kind: "ring",
    color,
    fillRatio: RING_FILL_BY_STATUS[normalized] ?? 0,
  };
}
...
/** Pie wedge path for status progress (same math as desktop/Next). */
