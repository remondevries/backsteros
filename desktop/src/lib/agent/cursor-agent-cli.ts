...
export type AgentAttachRequest = {
  ...
  focusUi?: boolean;
  /**
   * View / return-to-task: reconnect the disposable viewer to the existing
   * sidecar PTY. Resume is deferred until WS `ready` and skipped when the
   * agent is already live (avoids typing `agent --resume` into an open TUI).
   */
...
/** End a bound agent session: destroy the PTY, clear the task terminal UI, drop core binding. */
