...
fn resolve_desktop_dir() -> Result<PathBuf, String> {
    for key in ["BACKSTEROS_DESKTOP_DIR", "BACKSTEROS_APP_DIR"] {
        if let Ok(configured) = std::env::var(key) {
            let trimmed = configured.trim();
            if !trimmed.is_empty() {
    ...
    // src-tauri → desktop (owns scripts/whoop/fetch-day.mjs)
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let candidate = manifest_dir.join("..");
    let canonical = candidate
        .canonicalize()
        .map_err(|error| format!("Could not resolve desktop package: {error}"))?;
    if !canonical.is_dir() {
        return Err(format!(
...
pub fn whoop_fetch_day(date: String) -> Result<WhoopDayResult, String> {
    ...
    let app_dir = resolve_desktop_dir()?;
