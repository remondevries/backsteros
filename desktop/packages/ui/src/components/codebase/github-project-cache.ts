...
/** Drop branch/commit/PR caches for a project so focus-refresh can refetch. */
export function clearCachedGithubProjectData(projectId: string): void {
  const prefix = `${projectId}\0`;
  for (const key of [...branchesCache.keys()]) {
    if (key.startsWith(prefix)) branchesCache.delete(key);
  }
  for (const key of [...selectedBranchCache.keys()]) {
    if (key.startsWith(prefix)) selectedBranchCache.delete(key);
  }
  for (const key of [...commitsCache.keys()]) {
    if (key.startsWith(prefix)) commitsCache.delete(key);
  }
  for (const key of [...pullRequestsCache.keys()]) {
    if (key.startsWith(prefix)) pullRequestsCache.delete(key);
  }
}
