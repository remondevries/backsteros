...
import { getSelectedProjectDocumentPathFromPathname } from "../../compose/compose-document-folders.js";
import { DOCUMENT_TREE_CREATE_FOLDER_SHORTCUT_HINT } from "../../documents/should-handle-document-tree-create-folder-shortcut.js";
import { AddFolderInline } from "../documents/add-folder-inline.js";
...
import { DocumentTreeNodeView } from "../documents/document-tree.js";
import { FolderPlusIcon } from "../icons/folder-plus-icon.js";
import { SidePanelPlusIcon } from "../shell/side-panel-plus-icon.js";

export type ProjectDocumentsSidePanelLinkComponent = ComponentType<{
  to: string;
  className?: string;
  "aria-current"?: "page";
  children: ReactNode;
...
export type ProjectDocumentsSidePanelMutationResult =
  | { ok: true }
  | { ok: false; error: string };

export type ProjectDocumentsSidePanelViewProps = {
  pathname: string;
  items: KnowledgeListItem[];
  Link: ProjectDocumentsSidePanelLinkComponent;
  getDocumentHref: (pathOrId: string) => string;
  onAdd?: (parentFolderId: string | null) => void;
  onCreateFolder?: (input: {
    title: string;
    parentId: string | null;
  }) => Promise<ProjectDocumentsSidePanelMutationResult>;
  onRename?: (
    id: string,
    title: string,
  ) => Promise<ProjectDocumentsSidePanelMutationResult>;
...
/**
 * Chrome content side panel for project documents — mirrors KnowledgeSidePanelView
 * with project-scoped hrefs and a Documents header.
 */
export function ProjectDocumentsSidePanelView({
  pathname,
  items,
  Link,
  getDocumentHref,
