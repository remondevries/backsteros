...
export type ResizableSidePanelProps = {
  children: ReactNode;
  storageKey: string;
  defaultWidth?: number;
  minWidth?: number;
...
/**
 * Right (or left) resizable entity properties rail — matches web ResizableSidePanel.
 */
export function ResizableSidePanel({
  ...
  const handlePointerDown = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      if (!isDesktop || fillAvailableSpace) return;
    ...
    [edge, fillAvailableSpace, isDesktop, minWidth, persistWidth, resolveMaxWidth],
  );
  ...
  return (
    <aside
      ref={panelRef}
      className={`resizable-side-panel${className ? ` ${className}` : ""}${
    ...
    >
      {isDesktop && !fillAvailableSpace ? (
        <div
          ...
          className={`resizable-side-panel__handle resizable-side-panel__handle--${edge}`}
        >
          <span
            aria-hidden="true"
      ...
      <div className="resizable-side-panel__body">{children}</div>
    </aside>
  );
}
