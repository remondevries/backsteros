import type { ContactSocialAccount } from "../components/contacts/contact-social-accounts-editor.js";
import type { ContactListItem } from "../navigation/entity-routes.js";
import { contactMatchesSlug } from "../navigation/entity-routes.js";

export type SocialContactListItem = ContactListItem & {
...
export function getSocialHref(numberOrId?: number | string): string {
  if (numberOrId == null) return "/social";
  return `/social/${numberOrId}`;
}

export function getSelectedSocialSlugFromPathname(
  pathname: string,
): string | null {
  const match = pathname.match(/^\/social\/([^/]+)/);
  return match ? decodeURIComponent(match[1]!) : null;
}

export function isSocialSectionPath(pathname: string): boolean {
  return pathname === "/social" || pathname.startsWith("/social/");
}

export function socialContactMatchesSlug(
  contact: SocialContactListItem,
  slug: string | null,
): boolean {
  return contactMatchesSlug(contact, slug);
}

/** Prefer LinkedIn, then X, then first account — for list avatar badge. */
