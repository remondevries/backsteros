...
import { buildCalendarDayHabitsByDate } from "../dist/calendar-day-habits.js";
...
test("buildCalendarDayHabitsByDate groups habit tasks by due date", () => {
  ...
  assert.equal(aug23?.length, 2);
  assert.equal(aug23?.[0]?.habitId, "h1");
  assert.equal(aug23?.[1]?.habitId, "h2");
});
