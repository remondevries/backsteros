...
export function getOverflowScrollParent(node) {
...
export function expandRevealTopForAdjacentHeader(args) {
    const { targetTop, headerTop, headerBottom, maxGapPx = KEYBOARD_NAV_ADJACENT_HEADER_GAP_PX, } = args;
...
function isStickyOrFixed(style) {
    return style.position === "sticky" || style.position === "fixed";
}
function shouldIgnoreStickyCover(sticky, target) {
 ...
 */
export function measureStickyCoverInsets(scrollParent, target, stickySelector = KEYBOARD_NAV_STICKY_COVER_SELECTOR) {
    const parentRect = scrollParent.getBoundingClientRect();
    let topInset = 0;
    let bottomInset = 0;
    for (const node of scrollParent.querySelectorAll(stickySelector)) {
        ...
        if (rect.bottom >= stuckBottomEdge - 1 &&
...
function headerInNode(node, headerSelector) {
    ...
    return node.querySelector(headerSelector);
}
/**
 ...
 */
export function findPrecedingSectionHeader(target, scrollParent, headerSelector = KEYBOARD_NAV_SECTION_HEADER_SELECTOR) {
    const owningGroup = target.closest(".project-type-subgroup, .status-group-section");
    if (owningGroup instanceof HTMLElement &&
        scrollParent.contains(owningGroup)) {
        const inGroup = owningGroup.querySelector(headerSelector);
 ...
 */
export function resolveKeyboardNavScrollTarget(marker) {
    if (marker.classList.contains("keyboard-nav-item-highlight") ||
        marker.classList.contains("keyboard-nav-list-item")) {
        return marker;
    }
    return (marker.querySelector(".keyboard-nav-item-highlight") ??
        marker.querySelector(".keyboard-nav-list-item") ??
        marker);
}
function revealTopForTarget(target, scrollParent) {
 ...
 */
export function scrollElementIntoViewPastSticky(target) {
