...
let suppressOriginClientX = null;
let suppressOriginClientY = null;
let lastHoveredKeyboardNavItemId = null;
let onMouseResumeKeyboardNav = null;
/** Called when pointer interaction clears keyboard-nav hover suppression. */
export function setKeyboardNavMouseResumeHandler(handler) {
...
function keyboardNavItemIdFromElement(element) {
    if (!element) {
        return null;
    }
...
export function getLastHoveredKeyboardNavItemId() {
    if (hasPointerPosition) {
        const underPointer = document.elementFromPoint(lastPointerClientX, lastPointerClientY);
...
export function isKeyboardNavHoverSuppressed() {
    return (document.body.getAttribute(KEYBOARD_NAV_HOVER_ATTR) ===
 ...
 */
export function resolveListKeyboardAnchorId(highlightedId, selectedId, itemIds) {
    const highlightedInList = highlightedId != null && itemIds.includes(highlightedId)
        ? highlightedId
        : null;
    if (highlightedInList != null) {
        return highlightedInList;
    }
    const selectedInList = selectedId != null && itemIds.includes(selectedId) ? selectedId : null;
    if (isKeyboardNavHoverSuppressed()) {
        return selectedInList;
    }
    const hoveredId = getLastHoveredKeyboardNavItemId();
    const hoveredInList = hoveredId != null && itemIds.includes(hoveredId) ? hoveredId : null;
    return hoveredInList ?? selectedInList;
}
export function suppressKeyboardNavHover() {
    document.body.setAttribute(KEYBOARD_NAV_HOVER_ATTR, KEYBOARD_NAV_HOVER_SUPPRESSED);
...
export function clearKeyboardNavHoverSuppression() {
...
function pointerMovedBeyondThreshold(clientX, clientY) {
...
export function installKeyboardNavHoverModalityListeners() {
