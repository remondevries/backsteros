export function stepListKeyboardIndex(currentIndex, direction, length) {
    if (length === 0) {
        return -1;
    }
    if (direction === "down") {
...
export function resolveListKeyboardStepTarget(input) {
    const { direction, highlightedId, selectedId, itemIds, hoverAnchorId } = input;
    if (itemIds.length === 0)
        return null;
    const hasHighlight = highlightedId != null && itemIds.includes(highlightedId);
    if (hasHighlight) {
    ...
    const hoverInList = hoverAnchorId != null && itemIds.includes(hoverAnchorId)
        ? hoverAnchorId
        : null;
    if (hoverInList != null) {
...
export function flattenGroupedListItemIds(groups, collapsedKeys, getItemId) {
    const result = [];
