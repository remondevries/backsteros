import { isBlockingModalOpen, isEditableShortcutTarget, } from "../shortcuts/shortcut-guards.js";
/** True while a SearchableDropdown menu panel (or expanded trigger) is open. */
export function isSearchableDropdownMenuOpen() {
    if (typeof document === "undefined")
        return false;
    if (document.querySelector("[data-searchable-dropdown-panel]")) {
        return true;
    }
    return (document.querySelector('[data-searchable-dropdown-root] [aria-expanded="true"]') !== null);
}
/**
 * Escape — clear multi-select on list rows (tasks, transactions).
 * Yields to modals, open dropdowns, and editable fields.
 */
export function shouldHandleClearSelectionShortcut(event, enabled) {
    if (!enabled)
        return false;
