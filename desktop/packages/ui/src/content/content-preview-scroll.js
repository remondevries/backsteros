...
import { BLOCKING_MODAL_SELECTOR, isBlockingModalOpen, shouldHandleGlobalShortcut, } from "../shortcuts/shortcut-guards.js";
...
function shouldHandleGlobalShortcutForContentPreview(event) {
    ...
    const container = document.querySelector(CONTENT_PREVIEW_SCROLL_SELECTOR);
    return container?.closest(BLOCKING_MODAL_SELECTOR) != null;
}
export function findContentPreviewScrollContainer() {
    if (!isContentPreviewModeActive()) {
        return null;
    }
    return document.querySelector(CONTENT_PREVIEW_SCROLL_SELECTOR);
}
export function contentPreviewHasVerticalOverflow(container) {
    return container.scrollHeight > container.clientHeight + 1;
}
export function getContentPreviewScrollStep(container) {
    const viewportStep = Math.round(container.clientHeight * SCROLL_STEP_VIEWPORT_RATIO);
    return Math.min(MAX_SCROLL_STEP_PX, Math.max(MIN_SCROLL_STEP_PX, viewportStep));
}
export function shouldHandleContentPreviewArrowScroll(event) {
    const key = event.key;
...
export function scrollContentPreviewByArrowKey(event) {
