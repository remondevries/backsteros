...
/** Skip view-mode toggle when the double-click landed on an interactive control. */
export function shouldToggleContentViewModeOnDoubleClick(
  ...
  if (
    mode === "edit" &&
    target.closest(".cm-content, input, textarea, select, [contenteditable='true']")
  ) {
    return false;
  }

  if (mode === "preview" && target.closest("a, button, input, textarea, select, [role='button']")) {
    return false;
  }
