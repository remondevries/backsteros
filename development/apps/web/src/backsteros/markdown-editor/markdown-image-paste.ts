...
export type UploadMarkdownImages = (files: File[]) => Promise<string[] | null | undefined>;

/** Collect image files from a paste or drop DataTransfer. */
export function collectImageFiles(data: DataTransfer | null | undefined): File[] {
  if (!data) return [];
  return [...(data.files ?? [])].filter((file) => file.type.startsWith("image/"));
}

/** Build a markdown image line for an uploaded content URL. */
export function markdownImageSnippet(url: string, alt = "screenshot"): string {
  ...
  return [
    EditorView.domEventHandlers({
      paste(event, view) {
        ...
        if (files.length === 0 || !getUpload()) return false;
