...
export default function Sidebar() {
  ...
  // Log mode (toggle on) shows BacksterOS projects/tasks; vibe mode is normal T3 threads.
  ...
  const backsterosRailMode = useSidebarModeStore((state) => state.backsterosRailMode);
  ...
  const captureCurrentSidebarModeLocation = useCallback((): SidebarModeResumeLocation => {
    return captureSidebarModeResumeLocation({
      draftId: routeTarget?.kind === "draft" ? routeTarget.draftId : null,
      environmentId: routeThreadRef?.environmentId ?? null,
      threadId: routeThreadRef?.threadId ?? null,
      ...
      switch (location.kind) {
        ...
        case "draft":
          void router.navigate({
            to: "/draft/$draftId",
            params: { draftId: DraftId.make(location.draftId) },
          });
          return;
        case "thread":
          void router.navigate({
            ...
            params: buildThreadRouteParams(
              scopeThreadRef(
                location.environmentId as EnvironmentId,
                location.threadId as ThreadId,
              ),
            ),
          });
          return;
        case "backsteros-project":
          void router.navigate({
            ...
            search: location.title?.trim() ? { title: location.title } : {},
          });
          return;
      }
    },
  ...
  useEffect(() => {
    ...
    const pending = pendingLogTaskDetailRef.current;
    if (!pending) return;
    restoreLogTaskDetail(pending);
  }, [isBacksterosScope, restoreLogTaskDetail]);

  // After a reload in log mode, reopen the remembered task detail (UI store is not persisted).
  useEffect(() => {
    if (didRestoreLogTaskDetailOnMountRef.current) return;
    if (!logModeEnabled || !savedLogTaskDetail) return;
    if (backsterosTaskSelection) {
  ...
  useEffect(() => {
    clearThreadSearch();
  }, [clearThreadSearch, projectScopeKey, logModeEnabled]);

  const backsterosListMode =
    isBacksterosScope && backsterosTaskSelection?.taskId != null
      ? "tasks"
      : isBacksterosScope
        ? backsterosRailMode
        : "projects";
  useEffect(() => {
    if (!isBacksterosScope) return;
    clearThreadSearch();
  }, [backsterosListMode, clearThreadSearch, isBacksterosScope]);
  ...
  const handleLogModeChange = useCallback(
    (checked: boolean) => {
      const currentLocation = captureCurrentSidebarModeLocation();
      const currentTaskDetail: SidebarModeTaskDetailResume | null = backsterosTaskSelection
        ? {
            taskId: backsterosTaskSelection.taskId,
            projectId: backsterosTaskSelection.project.id,
          }
        : null;
      ...
      if (checked) {
        // Entering log: remember vibe, restore last log place + task.
        if (currentLocation.kind !== "backsteros-project") {
          rememberVibeLocation(currentLocation);
        }
        ...
        if (restoreTaskDetail) {
          restoreLogTaskDetail(restoreTaskDetail);
        }
        return;
      }

      // Leaving log: remember log, restore last vibe place.
      rememberLogLocation(currentLocation, currentTaskDetail);
      setLogModeEnabled(false);
      pendingLogTaskDetailRef.current = null;
      clearTaskDetail();
      if (savedVibeLocation) {
        if (!sidebarModeResumeLocationsEqual(savedVibeLocation, currentLocation)) {
          navigateToSidebarModeResumeLocation(savedVibeLocation);
        }
        return;
      }
      if (currentLocation.kind === "backsteros-project" || routeBacksterosProjectId) {
        void router.navigate({ to: "/" });
      }
    },
    [
  ...
  const handleThreadSearchKeyDown = useCallback(
    (event: ReactKeyboardEvent<HTMLInputElement>) => {
      ...
      // BacksterOS filters the visible list in place — no result picker to walk.
      if (isBacksterosScope) return;
  ...
  const handleNewBacksterosTaskClick = useCallback(() => {
  ...
  const backsterosSearchTargetsTasks =
    backsterosRailMode === "inbox" || backsterosTaskSelection != null;
  const sidebarSearchPlaceholder = isBacksterosScope
    ? backsterosRailMode === "inbox"
      ? "Search inbox"
      : backsterosSearchTargetsTasks
        ? "Search tasks"
        : "Search projects"
    : "Search";
  const sidebarSearchAriaLabel = isBacksterosScope
    ? backsterosRailMode === "inbox"
      ? "Search BacksterOS inbox"
      : backsterosSearchTargetsTasks
        ? "Search BacksterOS tasks"
        : "Search BacksterOS projects"
    : "Search threads";
  return (
    <>
      <SidebarChromeHeader
            ...
            <div className="flex items-center gap-1">
              <div className="flex h-8 min-w-0 flex-1 items-center gap-2 rounded-md px-2 py-1.5 text-sm font-medium text-sidebar-muted-foreground hover:bg-sidebar-row-hover hover:text-sidebar-foreground">
                ...
                <Input
                  ...
                  role={isBacksterosScope ? "searchbox" : "combobox"}
                  aria-autocomplete={isBacksterosScope ? undefined : "list"}
                  aria-expanded={
                    isBacksterosScope
                      ? undefined
      ...
      >
        <SidebarGroup className="ps-[calc(var(--sidebar-content-inset)+1px)] pe-[var(--sidebar-content-inset)] pb-1 pt-0">
          <BacksterosContentCrossfade
            contentKey={isBacksterosScope ? "log" : "vibe"}
            className="flex min-h-0 flex-1 flex-col"
          >
            {(mode) =>
              mode === "log" ? (
                <BacksterosPanel searchQuery={threadSearchQuery} />
              ) : (
                <>
                  {isSearchingThreads ? (
