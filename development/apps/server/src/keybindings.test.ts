...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("ships configurable thread navigation defaults", () =>
    Effect.sync(() => {
      ...
      assert.equal(defaultsByCommand.get("composer.focus"), "mod+m");
      assert.equal(defaultsByCommand.get("composer.blur"), "escape");
      ...
      assert.equal(defaultsByCommand.get("sidebar.toggle"), "[");
      assert.equal(defaultsByCommand.get("taskDetail.toggle"), "shift+[");
      assert.equal(defaultsByCommand.get("rightPanel.toggle"), "]");
