...
import {
  ...
  getProjectRouteParamFromDocumentsPath,
  getSelectedDocumentPathFromPathname,
} from "@/lib/document-navigation-path";
import {
  buildDocumentTree,
...
export function ProjectDocumentsSidePanel({ pathname }: { pathname: string }) {
  ...
  const projectRouteParam = getProjectRouteParamFromDocumentsPath(pathname);
  const selectedPath = getSelectedDocumentPathFromPathname(pathname);
  ...
  const local = usePowerSyncQuery<Record<string, unknown>>(
    projectRouteParam
      ? "SELECT * FROM documents WHERE deleted_at IS NULL AND type = 'project' AND (project_id = ? OR project_id IN (SELECT id FROM projects WHERE lower(key) = lower(?))) ORDER BY sort_order, path, updated_at DESC"
      : null,
    projectRouteParam ? [projectRouteParam, projectRouteParam] : [],
  );
  ...
  const resolvedProjectId = useMemo(
    () => documents.find((document) => document.projectId)?.projectId ?? projectRouteParam,
    [documents, projectRouteParam],
  );
  ...
  const { highlightedId } = useListKeyboardNavigation({
    ...
    onNavigate: (itemId) => {
      ...
      if (!projectRouteParam) return;
      const document = documents.find((entry) => entry.id === itemId);
      if (!document) return;
      const href = getProjectDocumentHref(projectRouteParam, document.path || document.id);
      if (href !== pathname) {
        router.push(href);
      }
    },
    zone: LIST_KEYBOARD_NAV_ZONE_SIDE_PANEL,
    enabled: Boolean(projectRouteParam) && itemIds.length > 0,
  });
  ...
  const handleCreateDocument = useCallback(() => {
    if (!projectRouteParam || !resolvedProjectId) return;
    startCreateTransition(async () => {
      ...
      try {
        ...
        resource.reload();
        router.push(getProjectDocumentHref(projectRouteParam, created.path || created.id));
      } catch (error) {
        toast.error(error instanceof Error ? error.message : "Could not create document.");
      }
    });
  }, [client, projectRouteParam, resolvedProjectId, resource, router, selectedFolderId]);
          ...
          request.fromParentId !== request.toParentId
            ...
            : await reorderDocumentEntriesAction({
                orderedIds: (() => {
                  ...
                  const siblings =
                    parent === null ? tree : parent.type === "folder" ? parent.children : [];
                  const ids = siblings
  ...
  if (!projectRouteParam) {
    return <p className="app-content-side-panel-empty">No project context.</p>;
  }
  ...
  return (
    <div className="app-content-side-panel app-content-side-panel--documents flex h-full min-h-0 flex-col">
      ...
      <div className="app-content-side-panel-main flex min-h-0 flex-1 flex-col">
            ...
            <AddFolderInline
              ...
              onSubmit={async (name) => {
                const result = await createProjectFolderAction({
                  projectId: resolvedProjectId ?? projectRouteParam,
                  title: name,
        ...
        {!documents.length && !addingFolder ? null : (
          ...
          >
            {tree.map((node) => (
              <DocumentTreeNodeView
                ...
                getDocumentHref={(path) => getProjectDocumentHref(projectRouteParam, path)}
