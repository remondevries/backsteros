...
import type { HistoryEntryDisplay } from "@/lib/navigation-history/resolve-history-entry-display";
...
export function HistoryEntryIcon({
  ...
  if (display.kind === "journal") {
    if (icon != null) {
      return (
        <span className="app-side-panel-history-entry-icon" aria-hidden="true">
          <DocumentOcticon icon={icon} size={14} className={ICON_CLASS} />
        </span>
      );
    }
  ...
  if (display.kind === "letter") {
    return (
      <span className="app-side-panel-history-entry-icon" aria-hidden="true">
        <LetterOcticon icon={icon} size={14} className={ICON_CLASS} />
      </span>
    );
  }
