...
/** Minimum set for spinner + turn detection (same events Orca uses for Cursor). */
const CURSOR_EVENTS = [
  "beforeSubmitPrompt",
  "stop",
  "sessionEnd",
...
function isOurCommand(command) {
  ...
  return command.includes(SCRIPT_NAME) || command.includes("backsteros-development/scripts/cursor-agent-hook");
}
...
export function ensureCursorAgentHooks() {
  ...
  for (const eventName of CURSOR_EVENTS) {
    ...
    nextHooks[eventName] = [...withoutOurs, { command, timeout: 10 }];
    changed = true;
  }
