...
import { formatContactDisplayId } from "@/lib/contact-display-id";
import { formatOrganizationDisplayId } from "@/lib/organization-display-id";
import {
  parseContactSlug,
  parseLetterSlug,
...
import type { NavigationShortcutId } from "@/lib/shortcuts/navigation-shortcut-bindings";
import { formatTaskDisplayId } from "@/lib/task-display-id";
import {
  isTasksDueFilter,
...
const TOP_LEVEL_NAV: Record<
  string,
  { navId: NavigationShortcutId; badgeLabel: string }
> = {
...
export function resolveHistoryEntryDisplay(
  ...
  if (inboxDetailMatch) {
    ...
    if (taskParsed) {
      return {
        kind: "task",
        badgeLabel: formatTaskDisplayId(taskParsed.contextKey, taskParsed.number),
        title,
      };
    }
  ...
  const projectTaskMatch = pathname.match(/^\/projects\/[^/]+\/tasks\/([^/]+)$/);
  if (projectTaskMatch) {
  ...
  const contactTaskMatch = pathname.match(/^\/contacts\/[^/]+\/tasks\/([^/]+)$/);
  if (contactTaskMatch) {
  ...
  if (isProjectDocumentDetailPath(pathname) || isKnowledgeDocumentDetailPath(pathname)) {
    return {
      kind: "document",
      badgeLabel: "Document",
      title,
