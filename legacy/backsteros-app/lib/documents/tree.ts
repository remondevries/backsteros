...
/** Builds a folder/document tree from BOS's flat `documents` rows using `parentId`/`kind`. */
export function buildDocumentTree(documents: ApiDocument[]): DocumentTreeNode[] {
...
export function countDocumentTreeFolderItems(folder: DocumentTreeFolderNode): number {
  let count = 0;
