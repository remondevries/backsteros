...
import { isEntitySectionListPathname } from "@/lib/shortcuts/entity-section-list-path";
...
export function stepListKeyboardNavZone(
  ...
  const nextIndex =
    (startIndex + step + ordered.length) % ordered.length;
...
export function resolveListKeyboardNavTabTargetZone(
  ...
  if (!currentHasItems) {
    return stepListKeyboardNavZoneFromUnavailable(
      current,
      direction,
      available,
    );
