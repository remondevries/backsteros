...
import { registerActiveListKeyboardItemResolver } from "@/lib/shortcuts/active-list-keyboard-item";
import { registerFocusedListKeyboardItemResolver } from "@/lib/shortcuts/focused-list-keyboard-item";
import {
  focusListKeyboardNavItem,
  scrollKeyboardNavItemIntoView,
} from "@/lib/shortcuts/keyboard-nav-item";
import { boardKeyboardNavDirection } from "@/lib/shortcuts/board-keyboard-nav";
import {
...
} from "@/lib/shortcuts/keyboard-nav-hover-modality";
import { KEYBOARD_NAV_ITEM_ATTR } from "@/lib/shortcuts/keyboard-nav-item";
import { stepListKeyboardIndex } from "@/lib/shortcuts/list-keyboard-nav-index";
import {
...
} from "@/lib/shortcuts/should-handle-list-keyboard-navigation";

/** @deprecated Prefer `zone` on `useListKeyboardNavigation`. */
export const LIST_KEYBOARD_NAV_SIDE_PANEL_PRIORITY = 10;
/** @deprecated Prefer `LIST_KEYBOARD_NAV_CONTENT_PRIORITY` in list-keyboard-nav-zone. */
export { LIST_KEYBOARD_NAV_CONTENT_PRIORITY } from "@/lib/shortcuts/list-keyboard-nav-zone";
/** @deprecated Prefer `zone` on `useListKeyboardNavigation`. */
...
const mobileListKeyboardNavigationFallback: ListKeyboardNavigationContextValue =
  {
    register: () => () => {},
    setActiveZone: () => {},
    activeZone: null,
...
function useListKeyboardNavigationContext(): ListKeyboardNavigationContextValue {
  const context = useContext(ListKeyboardNavigationContext);
  if (context) {
    return context;
  }

  if (isMobileShellBuildActive()) {
    return mobileListKeyboardNavigationFallback;
  }

  throw new Error(
    "useListKeyboardNavigation must be used within ListKeyboardNavigationProvider",
  );
}
...
function hasListItems(registration: ListKeyboardNavigationRegistration): boolean {
...
export function pickActiveListKeyboardRegistration(
  ...
  const visible = registrations.filter(
    (registration) => isRegistrationVisible(registration) && hasListItems(registration),
  );
  if (visible.length === 0) {
    return null;
  }
...
export function ListKeyboardNavigationProvider({
  ...
  const applyActiveZone = useCallback(
    (zone: ListKeyboardNavZone, options?: ApplyListKeyboardNavZoneOptions) => {
      ...
      preferSidepanelForJkRef.current =
        options?.preferSidepanelForJk ?? false;
  ...
  useEffect(() => {
    ...
    const zoneChanged = lastPathnameZoneRef.current !== preferredZone;
  ...
  const register = useCallback(
    (registration: ListKeyboardNavigationRegistration) => {
      ...
      requestAnimationFrame(() => {
        ...
        if (
          activeZone &&
          !zoneHasNavigableItems(registrationsRef.current, activeZone)
        ) {
          syncActiveZoneToAvailableRegistrations(
            registrationsRef.current,
            activeZone,
  ...
  const setActiveZone = applyActiveZone;

  return (
    <ListKeyboardNavigationContext.Provider
      value={{ register, setActiveZone, activeZone }}
    >
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (registration.resolveNextItemId) {
        ...
        if (direction && isListNav) {
          const currentIndex =
            anchorId != null ? itemIds.indexOf(anchorId) : -1;
...
export function useListKeyboardNavigation({
  ...
  // Keyboard ring is j/k-only. Route selection stays separate (e.g. side-panel
  // active styles); selectedId is still used as a j/k anchor via getSelectedId.
