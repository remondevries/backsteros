...
export function normalizeAppTimezone(
  ...
  const trimmed = value?.trim();
  if (trimmed && isValidAppTimezone(trimmed)) return trimmed;
  return DEFAULT_APP_TIMEZONE;
}

export function appTimezoneLabel(value: string): string {
  const match = APP_TIMEZONE_OPTIONS.find((option) => option.value === value);
