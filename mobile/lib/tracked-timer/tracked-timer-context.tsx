...
function storedSecondsForEntry(entry: TimerEntry): number {
  ...
  return Math.max(memorySeconds(entry), resolvedSecondsFromFields(entry));
}
...
export function TrackedTimerProvider({
  ...
  const checkpointEntry = useCallback((entry: TimerEntry, persist = true) => {
  ...
  const pauseTimer = useCallback(
    (key: string) => {
      ...
      commitEntry(entry);
      syncRunningState(
        primaryRunningKeyRef.current === key
          ? null
          : primaryRunningKeyRef.current,
      );
      bumpEntries();
      setTick((value) => value + 1);
    },
  ...
  const registerTimer = useCallback(
    (registration: TrackedTimerRegistration) => {
      ...
      const next: TimerEntry = {
        ...
        trackedMinutes:
          registration.trackedMinutes ?? existing?.trackedMinutes ?? null,
        scheduleMinutes:
          registration.scheduleMinutes ?? existing?.scheduleMinutes ?? null,
      ...
      const metadataChanged =
        ...
        existing.trackedMinutes !== next.trackedMinutes ||
        existing.scheduleMinutes !== next.scheduleMinutes ||
        existing.sessionStartAt !== next.sessionStartAt ||
        existing.trackedDurationSeconds !== next.trackedDurationSeconds ||
        existing.accumulatedSeconds !== next.accumulatedSeconds;
