/**
 * Regenerates `lib/generated/pierre-file-icons.ts` from `@pierre/trees`
 * (complete sprite + token maps) plus desktop T3 exact-name overrides.
 * Resolves `@pierre/trees` from desktop / the pnpm store — not a mobile runtime dep.
 *
 * Run: `pnpm --filter @backsteros/mobile generate:pierre-icons`
 */
...
function findPierreTreesPkg() {
  const candidates = [
    path.join(mobileRoot, "node_modules/@pierre/trees"),
    path.join(workspaceRoot, "desktop/node_modules/@pierre/trees"),
    path.join(workspaceRoot, "desktop/packages/ui/node_modules/@pierre/trees"),
    path.join(workspaceRoot, "node_modules/@pierre/trees"),
  ];
  ...
  if (fs.existsSync(pnpmRoot)) {
    for (const entry of fs.readdirSync(pnpmRoot)) {
      if (!entry.startsWith("@pierre+trees@")) continue;
  ...
  throw new Error(
    "Could not find @pierre/trees. Install desktop deps (or keep generated icons).",
  );
}

const treesPkg = findPierreTreesPkg();
const root = path.join(treesPkg, "dist");
const builtIn = await import(pathToFileURL(path.join(root, "builtInIcons.js")).href);
const { createFileTreeIconResolver } = await import(
  pathToFileURL(path.join(root, "render/iconResolver.js")).href
);

/** Keep in sync with `desktop/packages/ui/src/pierre-icons.ts`. */
...
function parseSprite(sheet, into) {
  ...
  while ((match = re.exec(sheet)) != null) {
    ...
    if (!id) continue;
    const viewBox =
      /\bviewBox\s*=\s*["']([^"']+)["']/i.exec(attrs)?.[1] ?? "0 0 16 16";
    into[id] = { viewBox, body };
  }
}
...
function extractConstSet(src, name) {
  const start = src.indexOf(`const ${name} = `);
  if (start < 0) throw new Error(`missing ${name}`);
  const open = src.indexOf("[", src.indexOf("new Set([", start));
  let depth = 0;
  ...
  throw new Error(`unclosed set ${name}`);
}

const symbols = {};
parseSprite(builtIn.getBuiltInSpriteSheet("complete"), symbols);
parseSprite(T3_FILE_ICON_SPRITE, symbols);

const builtInSrc = fs.readFileSync(path.join(root, "builtInIcons.js"), "utf8");
const fileNameTokens = extractConstObject(builtInSrc, "BUILT_IN_FILE_NAME_TOKENS");
const extensionTokens = extractConstObject(
  builtInSrc,
  "BUILT_IN_FILE_EXTENSION_TOKENS",
);
const completeOverrides = extractConstObject(
  builtInSrc,
  "COMPLETE_EXTENSION_OVERRIDES",
);
const standardTier = extractConstSet(builtInSrc, "STANDARD_TIER_TOKENS");

const resolver = createFileTreeIconResolver({
  set: "complete",
  colored: true,
  spriteSheet: T3_FILE_ICON_SPRITE,
  byFileName: T3_BY_FILE_NAME,
});
...
function resolveLocal(filePath) {
  ...
  const t3 = T3_BY_FILE_NAME[lower];
  if (t3) return { name: t3 };
  const fileNameToken = fileNameTokens[lower];
  if (fileNameToken) {
    return { name: `file-tree-builtin-${fileNameToken}`, token: fileNameToken };
  }
  for (const ext of getExtensionCandidates(fileName)) {
    const override = completeOverrides[ext];
    if (override) {
      return { name: `file-tree-builtin-${override}`, token: override };
    }
    const match = extensionTokens[ext];
    if (match) return { name: `file-tree-builtin-${match}`, token: match };
  }
  return { name: "file-tree-builtin-default", token: "default" };
}
...
for (const sample of samples) {
  const expected = resolver.resolveIcon("file-tree-icon-file", sample);
  const actual = resolveLocal(sample);
  if (
    expected.name !== actual.name ||
    (expected.token ?? undefined) !== (actual.token ?? undefined)
  ) {
    mismatches += 1;
    console.error("MISMATCH", sample, expected, actual);
  }
}
if (mismatches > 0) {
  process.exitCode = 1;
  throw new Error(`${mismatches} resolver parity mismatch(es)`);
}

const pkgVersion = JSON.parse(
  fs.readFileSync(path.join(treesPkg, "package.json"), "utf8"),
).version;

const outPath = path.join(mobileRoot, "lib/generated/pierre-file-icons.ts");
const header = `/* eslint-disable */
/**
 * AUTO-GENERATED by scripts/generate-pierre-file-icons.mjs — do not edit.
 * Source: @pierre/trees@${pkgVersion} (complete set) + desktop T3 overrides.
 */
`;

const body = `${header}
export type PierreSpriteSymbol = { viewBox: string; body: string };

export const PIERRE_FILE_ICON_SYMBOLS: Record<string, PierreSpriteSymbol> = ${JSON.stringify(symbols, null, 2)} as const;

export const PIERRE_BUILT_IN_FILE_NAME_TOKENS: Record<string, string> = ${JSON.stringify(fileNameTokens, null, 2)};

export const PIERRE_BUILT_IN_FILE_EXTENSION_TOKENS: Record<string, string> = ${JSON.stringify(extensionTokens, null, 2)};

export const PIERRE_COMPLETE_EXTENSION_OVERRIDES: Record<string, string> = ${JSON.stringify(completeOverrides, null, 2)};

export const PIERRE_STANDARD_TIER_TOKENS = new Set<string>(${JSON.stringify(standardTier)});

export const PIERRE_T3_BY_FILE_NAME: Record<string, string> = ${JSON.stringify(T3_BY_FILE_NAME, null, 2)};
`;

fs.mkdirSync(path.dirname(outPath), { recursive: true });
fs.writeFileSync(outPath, body);

const staleJson = path.join(
  mobileRoot,
  "lib/generated/pierre-file-icon-symbols.json",
);
if (fs.existsSync(staleJson)) fs.unlinkSync(staleJson);

console.log(
  `Wrote ${path.relative(mobileRoot, outPath)} (${Object.keys(symbols).length} symbols)`,
);
