...
export function InboxCreateTaskScreen() {
  ...
  const dueFilterParam = Array.isArray(params.dueFilter)
    ? params.dueFilter[0]
    : params.dueFilter;
  const dueYmdParam = Array.isArray(params.dueYmd)
    ? params.dueYmd[0]
    : params.dueYmd;
  ...
  const initialDueDate = useMemo(() => {
    if (dueYmdParam && /^\d{4}-\d{2}-\d{2}$/.test(dueYmdParam)) {
      const [year, month, day] = dueYmdParam.split("-").map(Number);
      if (year && month && day) {
