...
function languageExtensionForPath(filePath: string): Extension | null {
  ...
  switch (ext) {
    ...
    case "htm":
      return html();
    // No dedicated Astro grammar in CodeMirror — HTML (+ nested JS/CSS) is
    // the closest match for the template body and component markup.
    case "astro":
...
/** Chrome + vim caret styling (matches document editor orange block). */
...
export function FileCodeViewer({
  ...
  const extensions = useMemo(() => {
    ...
    return [
      // Vim must be registered before other keymaps (see @replit/codemirror-vim).
      vim({ status: false }),
      relativeLineNumbers(),
  ...
  // Only focus when focusRequest bumps — not when the editor remounts with a
  // leftover counter (e.g. switching back to the Files tab).
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      // Let vim finish insert/visual → normal before leaving the editor.
