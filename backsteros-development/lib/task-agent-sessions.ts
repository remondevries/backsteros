const STORAGE_KEY = "backsteros-development.task-agent-sessions";
/** Chat ids the user destroyed — never resume/reattach these. */
const DESTROYED_CHATS_KEY =
...
function readDestroyedChatIds(): Set<string> {
  ...
  try {
    const raw = window.localStorage.getItem(DESTROYED_CHATS_KEY);
...
export const TASK_AGENT_SESSIONS_CHANGED_EVENT =
  "backsteros-development:task-agent-sessions-changed";
